...
import { Activity, AlertTriangle, ChevronDown, ChevronRight, Eye } from 'lucide-react';
import { syncApi } from '../services';
import type {
  DatabaseTriggersResponse,
  TableTrigger,
  TableTriggerIdentity,
} from '../types/api';

function triggerKey(t: TableTriggerIdentity): string {
...
export default function TableTriggersView({ envName, onCountLoaded, viewMode: externalViewMode, onViewModeChange }: {
  ...
  useEffect(() => {
    ...
    syncApi.getDatabaseTriggers(envName, includeInternal)
      .then(d => {
        ...
        const sKeys = new Set(sourceSide?.table_triggers.map(triggerKey) || []);
        const tKeys = new Set(targetSide?.table_triggers.map(triggerKey) || []);
        const allCount = new Set([...sKeys, ...tKeys]).size;
        onCountLoaded?.(allCount);
      })
      .catch(e => setError(e instanceof Error ? e.message : 'Failed to load'))
      .finally(() => setLoading(false));
  }, [envName, includeInternal]);
  ...
  const { sourceMap, targetMap, allKeys, missingInTargetSet, missingInSourceSet, differsSet, totalDiffs } = useMemo(() => {
    ...
    const sMap = new Map<string, TableTrigger>();
    const tMap = new Map<string, TableTrigger>();

    sourceSide?.table_triggers.forEach(t => sMap.set(triggerKey(t), t));
    targetSide?.table_triggers.forEach(t => tMap.set(triggerKey(t), t));

    const keys = Array.from(new Set([...sMap.keys(), ...tMap.keys()])).sort();
    const mitSet = new Set(data.comparison.table_triggers_missing_in_target.map(triggerKey));
    const misSet = new Set(data.comparison.table_triggers_missing_in_source.map(triggerKey));
    const dSet = new Set(data.comparison.table_trigger_definition_differences.map(triggerKey));
  ...
  if (allKeys.length === 0) {
    return (
      <div className="space-y-3">
        <InternalToggle includeInternal={includeInternal} setIncludeInternal={setIncludeInternal} />
        <div className="bg-gray-900 border border-gray-800 rounded-xl px-5 py-10 text-center">
          ...
          <p className="text-sm text-gray-400">No table triggers found.</p>
        </div>
      </div>
    );
  }
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-gray-800/60">
              ...
              <th className="text-left px-4 py-2.5 text-gray-500 font-medium w-36">Table</th>
              <th className="text-left px-4 py-2.5 text-gray-500 font-medium w-20">Timing</th>
              <th className="text-left px-4 py-2.5 text-gray-500 font-medium w-28">Events</th>
          ...
          <tbody className="divide-y divide-gray-800/40">
            {filteredKeys.map(key => {
              ...
              const trigger = (st || tt) as TableTrigger;
...
function InternalToggle({ includeInternal, setIncludeInternal }: { includeInternal: boolean; setIncludeInternal: (v: boolean) => void }) {
  return (
    <button
...
function TriggerRow({ trigger, sourceTrigger, targetTrigger, missingTarget, missingSource, differs, hasDiff, isSelected, onToggle }: {
  trigger: TableTrigger;
  sourceTrigger?: TableTrigger;
  targetTrigger?: TableTrigger;
      ...
      >
        <td className="px-4 py-2">
          <div className="flex items-center gap-2">
            ...
            {trigger.is_internal && <span className="text-[9px] px-1 py-0.5 bg-gray-800 border border-gray-700 rounded text-gray-500">internal</span>}
          </div>
        </td>
        <td className="px-4 py-2 font-mono text-gray-400 text-[10px]">{trigger.schema}.{trigger.table}</td>
        <td className="px-4 py-2 text-gray-400">{trigger.timing}</td>
        <td className="px-4 py-2 text-gray-500 text-[10px]">{trigger.events.join(', ')}</td>
          ...
          <td colSpan={5} className="p-0">
            <div className="bg-gray-950 border-t border-gray-800 px-5 py-4 space-y-3">
              <div className="flex items-center gap-4 text-[10px] text-gray-500">
                <span>orientation: {trigger.orientation}</span>
                <span>fn: {trigger.function_schema}.{trigger.function_name}</span>
                {trigger.when_condition && <span className="italic truncate max-w-[200px]">WHEN {trigger.when_condition}</span>}
              </div>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
                <TriggerDefBlock label="Source" trigger={sourceTrigger} color="teal" />
                <TriggerDefBlock label="Target" trigger={targetTrigger} color="sky" />
              </div>
            </div>
          </td>
        </tr>
...
function TriggerDefBlock({ label, trigger, color }: {
  label: string;
  trigger?: TableTrigger;
  color: 'teal' | 'sky';
}) {
